...
- **`index.md`** holds ALL structure and name translations. Front matter: `title`, `description`, `image`, `categories`, `portion` (`{type: servings|units|area, value, unit}`), optional `author`, `defaultVariant`, `variants` (`[{key, name}]`), `tools` (`[{id, icon?, name}]`), and `ingredients` (`[{id, amount, unit, item, note?, group?, onlyForVariation?}]`). The text fields `item`, `note`, tool `name`, and variant `name` are either a plain string or an `{en, ca, es}` map (English required, others fall back to it). Both tools and ingredients can have optional `links` (`[{url, icon?, label}]`); `icon` on a tool defaults to `🔧`, on a link defaults to `🛒`.
