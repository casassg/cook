...
- **`index.md`** holds ALL structure and name translations. Front matter: `title`, `description`, `image`, `categories`, `portion` (`{type: servings|units|area, value, unit}`), optional `author`, `defaultVariant`, `variants` (`[{key, name}]`), `groups` (`[{key, name}]`), `tools` (`[{id, icon?, name, note?}]`), and `ingredients` (`[{id, emoji?, amount, unit, item, note?, group?, onlyForVariation?}]`). The text fields `item`, `note`, tool `name`, tool `note`, group `name`, and variant `name` are either a plain string or an `{en, ca, es}` map (English required, others fall back to it). Notes are Markdown and may contain links. Ingredient `group` is a plain string key referencing a declared group; groups render as section headers in first-encounter order; ungrouped ingredients are headerless. The `icon` on a tool defaults to `🔧`.
